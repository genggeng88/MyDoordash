export const login = (credentials) => {
  ...
  // fetch will return somthing called a Promise
...
export const getMenus = (restId) => {
  return fetch(`/restaurant/${restId}/menu`).then((response) => {
    if (response.status < 200 || response.status >= 300) {
      throw Error("Failed to get menus");
    }

    return response.json(); // reads the response body and parse it as JSON, returns a Promist that resolves with the result of parsing the body text as JSON
  });
};
...
export const getRestaurants = () => {
  return fetch("/restaurants/menu").then((response) => {
    if (response.status < 200 || response.status >= 300) {
      throw Error("Failed to get restaurants");
    }
...
export const getCart = () => {
  return fetch("/cart").then((response) => {
    if (response.status < 200 || response.status >= 300) {
      throw Error("Failed to get shopping cart data");
    }
    return response.json();
  });
};
